...
なお、画面左下に追加機能のための追加パッケージインストールの案内が出ていますが、インストールしておくとRustdeskを閉じてもバックグラウンドに常駐するようになります（Tailscale,　Dockerと同じ）。Rustdeskのウィンドウを起動しっぱなしにするのと同じですが、画面を占有されなくなるのでお好みでインストールしてください。
